...
function lastCarInArray(){
    var length = inventory.length-1;
    return inventory[length];
}                                                                

let lastCar = lastCarInArray();
...
function carInfoColumn(column, sorted=true) {
    var infoArray = [];
    for(let i=0; i<inventory.length; i++) {
        infoArray.push(inventory[i][column]);
    }
    if(sorted){infoArray = infoArray.sort();}
    return infoArray;
}

let carModels = carInfoColumn("car_model");
...
let carYears = carInfoColumn("car_year", false);
